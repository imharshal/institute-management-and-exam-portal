...
if (isset($_POST['btnEnroll']) and !isset($_SESSION['enrollmentSuccess'])) {
    ...
    $course = DB::queryFirstRow("SELECT fees,duration,discount,scholarship FROM courses WHERE id=%s", $courseid);
                        ...
                        ?>
                            <center>
                                <div class="alert alert-success mb-4" role="alert">
                                    <h3>Course Enrollment successfully.</h3>
                                ...
                                <button class="m-3 btn btn-primary" id="print" onclick="printJS('generatedForm', 'html')">Print/Download result</button>
...
<script>
    ...
    localStorage.setItem("ecs_registrationId", (rId != '') ? rId : localStorage.getItem("ecs_registrationId"));
    localStorage.setItem("ecs_enrollId", (eId != '') ? eId : localStorage.getItem("ecs_enrollId"));

    $.ajax({
        type: "POST",
        url: "../includes/admission_acknoledgement.php",
        data: {
            registrationId: localStorage.getItem("ecs_registrationId"),
            enrollmentId: localStorage.getItem("ecs_enrollId"),
            print: true
        },
